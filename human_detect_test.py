...
#   model_path = '/path/to/faster_rcnn_inception_v2_coco_2017_11_08/frozen_inference_graph.pb'
    ...
    num_bbox_correct = 0
    bbox_tracked_correct = 0
    num_bbox_total = 0
    
    #iterate over frames
    while True: 
        
        reidShouldveTracked = 0
        trackedCorrect = 0
        trackedIncorrect = 0
        reidNew = 0
        ...
        for i in range(len(boxes)):
            ...
            if classes[i] == 1 and scores[i] > threshold:
                ...
                ground_truth = odapi.version0(cropped_img)
        ...
        if(framenum % 10 == 0):    
            ...
            print('#bbox correct: '+str(num_bbox_correct))
            print('#bbox tracked correct: '+str(bbox_tracked_correct))
            print('#bbox : '+str(num_bbox_total))
            print(' ')
            pos = 100*(float(num_bbox_correct) / float(num_bbox_total) )
            pos_track = 100*(float( bbox_tracked_correct ) / float(num_bbox_total) )
            print('%bbox tracked correct: '+str( pos_track  ))
            print('%bbox correct: '+str( pos  ) + '\n\n--------------\n')
